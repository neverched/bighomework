				...
				<el-card class="box-card">
					<template #header>
						<div class="card-header">
							<el-select v-model="rankValue" class="m-2" placeholder="最新发布" @change="rankChange">
								<el-option v-for="item in rankOptions" :key="item.value" :label="item.label" :value="item.value" />
							</el-select>
							<el-button type="primary" plain id="create-ques" style="margin-left: 20px;" @click="toQuestionNew">
								<span>创建讨论</span>
								<el-icon>
									<Plus />
								</el-icon>
					...
					<MyItem v-for="q in questions" :key="q.id" :item="q" type="questions" />

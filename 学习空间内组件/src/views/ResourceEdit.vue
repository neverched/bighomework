<script setup>
  ...
  const giveOut = function(){
    ...
    formData.append('resource_name', inputTitle.value)
    formData.append('introduction', value.value)
    api.post('/api/spaces/1/resources/new',formData).then(
    res => {
      console.log(res)
...
<template>
  ...
  <el-cow>
    <ResourceHeader @save="onSaveClick" @setting="onSettingClick" @open-list="dialogVisible = true" @export="onExportClick" />
  </el-cow>
